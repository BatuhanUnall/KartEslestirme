document.addEventListener('DOMContentLoaded', () => {
    ...
    const bestTimeTxt = document.getElementById("hard-best-time")
    ...
    cards.forEach(value => {
        ...
        backFace.style.backgroundImage = `url('16images/${value}.jpg')`;
        ...
        if (firstCard.dataset.value === secondCard.dataset.value) {
            ...
            if (matchCount === 16) {
                ...
                if (totalMinutes == 0) {
                    ...
                    if((localStorage.getItem("hardBestTime") == null) || (totalSeconds < Number(localStorage.getItem("hardBestTime")))) {
                        bestTimeDiv.style.display = "block";
                        bestTimeTxt.addEventListener("change", function () {
                            localStorage.setItem("hardBestTime", totalSeconds)
                            saveBtn.addEventListener("click", function () {
                                localStorage.setItem("hardBestTimeName", bestTimeTxt.value)
                            })
                        })
                    }
                }
                    ...
                    if((localStorage.getItem("hardBestTime") == null) || (totalSeconds < Number(localStorage.getItem("hardBestTime")))) {
                        bestTimeDiv.style.display = "block";
                        bestTimeTxt.addEventListener("change", function () {
                            localStorage.setItem("hardBestTime", totalTime)
                            saveBtn.addEventListener("click", function () {
                                localStorage.setItem("hardBestTimeName", bestTimeTxt.value)
                            })
                        })
